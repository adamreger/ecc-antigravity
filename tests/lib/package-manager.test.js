...
function runTests() {
  ...
  if (test('respects environment variable', () => {
    ...
    try {
      process.env.ANTIGRAVITY_PACKAGE_MANAGER = 'yarn';
      const result = pm.getPackageManager();
      assert.strictEqual(result.name, 'yarn');
      assert.strictEqual(result.source, 'environment');
    } finally {
      if (originalEnv !== undefined) {
        process.env.ANTIGRAVITY_PACKAGE_MANAGER = originalEnv;
      } else {
        delete process.env.ANTIGRAVITY_PACKAGE_MANAGER;
      }
    }
  })) passed++;
  else failed++;
  ...
  if (test('returns correct install command', () => {
    const originalEnv = process.env.ANTIGRAVITY_PACKAGE_MANAGER;
    try {
      process.env.ANTIGRAVITY_PACKAGE_MANAGER = 'pnpm';
  ...
  if (test('returns correct test command', () => {
    const originalEnv = process.env.ANTIGRAVITY_PACKAGE_MANAGER;
    try {
      process.env.ANTIGRAVITY_PACKAGE_MANAGER = 'npm';
  ...
  if (test('returns informative prompt', () => {
    ...
    assert.ok(prompt.includes('ANTIGRAVITY_PACKAGE_MANAGER'), 'Should mention env var');
  ...
  if (test('sets project package manager', () => {
    ...
    try {
      ...
      const configPath = path.join(testDir, '.antigravity', 'package-manager.json');
  ...
  if (test('falls through on corrupted project config JSON', () => {
    ...
    const antigravityDir = path.join(testDir, '.antigravity');
    fs.mkdirSync(antigravityDir, { recursive: true });
    fs.writeFileSync(path.join(antigravityDir, 'package-manager.json'), '{not valid json!!!');
    const originalEnv = process.env.ANTIGRAVITY_PACKAGE_MANAGER;
    try {
      delete process.env.ANTIGRAVITY_PACKAGE_MANAGER;
      const result = pm.getPackageManager({ projectDir: testDir });
  ...
  if (test('falls through on project config with unknown PM', () => {
    ...
    const antigravityDir = path.join(testDir, '.antigravity');
    fs.mkdirSync(antigravityDir, { recursive: true });
    fs.writeFileSync(path.join(antigravityDir, 'package-manager.json'), JSON.stringify({ packageManager: 'nonexistent-pm' }));
    const originalEnv = process.env.ANTIGRAVITY_PACKAGE_MANAGER;
    try {
      delete process.env.ANTIGRAVITY_PACKAGE_MANAGER;
      const result = pm.getPackageManager({ projectDir: testDir });
  ...
  if (test('detects from valid project-config (.antigravity/package-manager.json)', () => {
    ...
    const antigravityDir = path.join(testDir, '.antigravity');
    fs.mkdirSync(antigravityDir, { recursive: true });
    fs.writeFileSync(path.join(antigravityDir, 'package-manager.json'), JSON.stringify({ packageManager: 'pnpm' }));
    const originalEnv = process.env.ANTIGRAVITY_PACKAGE_MANAGER;
    try {
      delete process.env.ANTIGRAVITY_PACKAGE_MANAGER;
      const result = pm.getPackageManager({ projectDir: testDir });
  ...
  if (test('project-config takes priority over package.json', () => {
    ...
    fs.writeFileSync(path.join(antigravityDir, 'package-manager.json'), JSON.stringify({ packageManager: 'bun' }));
  ...
  if (test('successfully saves preferred package manager', () => {
    ...
    const configPath = path.join(utils.getAntigravityDir(), 'package-manager.json');
  ...
  if (test('pnpm custom script: pnpm (no run keyword)', () => {
    ...
    try {
      ...
      if (originalEnv !== undefined) process.env.ANTIGRAVITY_PACKAGE_MANAGER = originalEnv;
      else delete process.env.ANTIGRAVITY_PACKAGE_MANAGER;
    }
  })) passed++;
  else failed++;
  ...
  if (test('bun custom script: bun run <script>', () => {
    const originalEnv = process.env.ANTIGRAVITY_PACKAGE_MANAGER;
    try {
      process.env.ANTIGRAVITY_PACKAGE_MANAGER = 'bun';
  ...
  if (test('ignores unknown env var package manager', () => {
    const originalEnv = process.env.ANTIGRAVITY_PACKAGE_MANAGER;
    try {
      process.env.ANTIGRAVITY_PACKAGE_MANAGER = 'totally-fake-pm';
      const result = pm.getPackageManager();
  ...
  if (test('setProjectPackageManager creates .antigravity directory if missing', () => {
    ...
    try {
      const antigravityDir = path.join(testDir, '.antigravity');
      assert.ok(!fs.existsSync(antigravityDir), '.antigravity should not pre-exist');
      pm.setProjectPackageManager('npm', testDir);
      assert.ok(fs.existsSync(antigravityDir), '.antigravity should be created');
      const configPath = path.join(antigravityDir, 'package-manager.json');
  ...
  if (test('getPackageManager falls through corrupted global config to npm default', () => {
    ...
    const origPM = process.env.ANTIGRAVITY_PACKAGE_MANAGER;

    try {
      // Create corrupted global config file
      const antigravityDir = path.join(tmpDir, '.antigravity');
      fs.mkdirSync(antigravityDir, { recursive: true });
      fs.writeFileSync(path.join(antigravityDir, 'package-manager.json'), '{ invalid json !!!', 'utf8');
      ...
      if (origPM !== undefined) process.env.ANTIGRAVITY_PACKAGE_MANAGER = origPM;
  ...
  if (test('getPackageManager returns source global-config when valid global config exists', () => {
    ...
    try {
      ...
      const antigravityDir = path.join(tmpDir, '.antigravity');
      fs.mkdirSync(antigravityDir, { recursive: true });
      fs.writeFileSync(path.join(antigravityDir, 'package-manager.json'), JSON.stringify({ packageManager: 'pnpm', setAt: '2026-01-01T00:00:00Z' }), 'utf8');
  ...
  if (test('setPreferredPackageManager throws wrapped error when save fails', () => {
    ...
    const antigravityDir = path.join(isoHome, '.antigravity');
    fs.mkdirSync(antigravityDir, { recursive: true });
    ...
    try {
      ...
      delete require.cache[require.resolve('../../scripts/lib/utils')];
      const freshPm = require('../../scripts/lib/package-manager');
      // Make .antigravity directory read-only — can't create new files (package-manager.json)
      fs.chmodSync(antigravityDir, 0o555);
      assert.throws(() => {
        freshPm.setPreferredPackageManager('npm');
      }, /Failed to save package manager preference/);
    } finally {
      try {
        fs.chmodSync(antigravityDir, 0o755);
      } catch (_err) {
        /* best-effort */
      }
  ...
  if (test('setProjectPackageManager throws wrapped error when write fails', () => {
    ...
    const antigravityDir = path.join(isoProject, '.antigravity');
    fs.mkdirSync(antigravityDir, { recursive: true });
    // Make .antigravity directory read-only — can't create new files
    fs.chmodSync(antigravityDir, 0o555);
    try {
      assert.throws(() => {
  ...
  if (test('getPackageManager skips empty string ANTIGRAVITY_PACKAGE_MANAGER (falsy short-circuit)', () => {
    ...
    const originalEnv = process.env.ANTIGRAVITY_PACKAGE_MANAGER;
    try {
      process.env.ANTIGRAVITY_PACKAGE_MANAGER = '';
      const result = pm.getPackageManager();
